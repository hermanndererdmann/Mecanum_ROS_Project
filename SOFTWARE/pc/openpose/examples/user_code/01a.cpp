...
// ROS publisher for keypoints image
ros::Publisher keypointsImagePublisher;
    ...
    try
    {
        ...
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            ...
            if (!cvMat.empty())
            {
                ...
                sensor_msgs::ImagePtr keypointsImageMsg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", cvMat).toImageMsg();

                // Publish the keypoints image
                keypointsImagePublisher.publish(keypointsImageMsg);
            }
            else
            {
...
int tutorialApiCpp()
{
    try
    {
        ...
        // Create a ROS publisher for the keypoints image
        keypointsImagePublisher = nh.advertise<sensor_msgs::Image>("keypoints_image", 1);
